...
def evaluate_checkpoint(checkpoint: Path, bundle: Path, output: Path, *, split="valid", threshold=0.25):
    ...
    if read_json(bundle / "ontology.json")["classes"] != ["black-pawn", "white-pawn"]:
        raise ValueError("the MVP evaluator requires the ordered black-pawn / white-pawn ontology")
    ...
    for offset in range(0, len(reference["images"]), 4):
        ...
        with torch.no_grad():
            raw = module.model(torch.stack(tensors))
            results = module.postprocess(raw, torch.tensor(sizes))
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        ...
        if predictions:
            ...
            for k, name in enumerate(("black-pawn", "white-pawn")):
            ...
            metrics = {"AP": 0.0, "AP50": 0.0, "AP75": 0.0, "AR100": 0.0,
                       "per_class_AP": {"black-pawn": 0.0, "white-pawn": 0.0}}
