...
The user explicitly reopened the work: do not stop merely because a batch completes. A new persistent goal is active. `docs/RESEARCH_LOOP.json` is the durable hypothesis queue; `artifacts/continuous/` holds execution receipts. Current next experiment: construction full-frame resolution and independent rare-class sampling, with matched fully labeled controls. Prior research-v2 work below is completed and must not be resubmitted. Training/inference stay cloud-only; existing credits, $0 cash. New calls require bounded reservations and recorded call IDs. Check this header and queue before following older completion statements.

NOW RUNNING: six construction continuations, 2000updates each, seed20260919. Aware2x2(512/640 × uniform/repeat exposure), complete512uniform and640repeat. GPU smoke passed two real640px updates for BOTH arms with strict parent loading, unchangedPE24 and identical exposure digest. Client `scripts/run_continuous_scale.py` is running (log `artifacts/continuous/scale/runner.log`); it collects weights before separate GPU evaluation. Resume that script if the client ends; it reconnects saved calls. Never delete reservations/callIDs or resubmit orphan submissions. ProtocolSHA e3db1a7db4525801bb42b12da757e9838a4425a1522aad95828e471e490fed8a. CallIDs `artifacts/continuous/scale/runs/*/call.json`, state `artifacts/continuous/scale/state.json`. Modal app https://modal.com/apps/idident5967/main/deployed/coveragecv-continuous-scale. No new result is published yet.

Budget reconciliation: all12prior training runs completed in5743.05training seconds. Retain7credits for them (maximum resources × measured time +300s overhead EACH computes6.0513), keep the entire5.80ceilings for prior nontraining categories, and reserve6.92for this cycle, leaving.59buffer against confirmed20.31starting balance. Original ledgers preserved. Provider observed4.1876for prior fourapps; monthly API24.69metered/0billed. Observations are not a settled remaining balance. Full reconciliation and provider report live under `artifacts/continuous/`. No new volume/retries. The new6.92includes six1.00training calls, six.12evaluations and one.20smoke.

Validation so far: seven pure metadata/recovery tests passed in0.20s; Ruff/diff checks pass; independent read-only source audit found no critical execution bug; real GPU smoke passed. All local tests excluded model imports/compute. Source/dependency/image receipts retained. The reference archive is physically present in the shared cloud image, but training code does not read it; do not claim physical isolation. Parent histories differ(awareAlpha1/fullAlpha3), so isolate resolution/sampling effects within each parent arm. After scoring, record negatives too, check matched gains, then advance the active goal to the next hypothesis.
