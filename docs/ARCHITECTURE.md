...
The completed annotation audit read only the partial learner training view and complete validation references, checked their manifest hashes and inspected all 33 observed no-helmet training boxes plus all 11 validation boxes. It found sparse supervision, a size shift, inconsistent head geometry and specific annotation defects; it also recorded genuine detector errors. That audit involved no hidden training labels, test data, annotation edits or split changes. The subsequent acquisition simulation below is a separate, explicitly authorized use of published training labels. The immutable original benchmark remains intact.

## Simulated annotation acquisition

`training/acquisition.py` separates selection from label revelation. The selection interface accepts only a verified partial training view and detector predictions, with no oracle argument. It ranks eligible `unknown`/`positive_only` image/class pairs using maximum model confidence or seeded random ordering, and freezes exactly 30 queries per class. Both 150-query plans and their shared prediction/checkpoint/view identities are recorded before either plan is applied to published reference training annotations.

Application verifies ontology, image IDs, pixel hashes, declared coverage and the frozen plan. It reveals only the selected image/class pairs, preserves observed labels, and publishes a new immutable learner view with an acquisition audit. Reviewed pairs receive exhaustive or verified-absent state according to the published reference, with explicit simulated-review provenance; this is not a claim of independent human verification. Validation files are copied unchanged and test data are excluded. Source errors may remain in the published oracle.

Guided review exposes 265 boxes, increasing the observed total from 2,119 to 2,384; random review exposes 198, reaching 2,317. No-helmet increases from 33 to 84 versus 38. Both spend 150 simulated image/class reviews, but their acquired label counts differ. Review units are not measured annotator time or monetary cost; more boxes may require more annotation effort. This is a fixed-query-budget acquisition experiment, not a fixed-label-budget estimate of the coverage loss or demonstrated human-cost efficiency.

Guided and random arms continue the same original 4,000-update aware checkpoint for 2,000 fixed updates at 512px. A new complete-standard arm continues its original complete checkpoint; the existing 6,000-update ordinary-aware checkpoint is the zero-review control. Optimizer/recipe/seed/evaluator remain matched, with no in-training validation or checkpoint selection. `scripts/run_acquisition.py` collects and verifies all three cloud checkpoints before local scoring and reuses saved call IDs/receipts on restart.

All primary evaluations completed: guided **49.8486**, random **47.3432**, zero-review **48.0026** and complete-standard **52.4241** AP50:95. Guided gains 2.5054 points over random and 1.8460 over zero-review. One seed, unequal acquired-box counts and validation-guided design limit the claim.

The separate CPU-only interaction also completed using the unchanged original tiling and training-size class rule, without a parameter search. Routed AP50:95 is **51.6204 guided, 49.3254 random, 49.8821 zero-review and 53.7707 complete**. Guided AP50 is 92.2047. Guided gains 1.7384 points over zero-review with the same routing and 1.7718 over its own fresh full-frame result. Five image passes took 39.45 seconds versus 8.02 for a full-frame pass across 120 images in the recorded CPU run. The scope/provenance audit verified all 120 images, 717 boxes and five classes, exact semantic source selection and per-class metric consistency. This secondary result combines extra labels with added inference cost; the single-pass primary remains unchanged.

The reusable local commands are `plan-reviews VIEW PREDICTIONS CHECKPOINT OUTPUT` and `simulate-reviews VIEW PUBLISHED_REFERENCE PLAN OUTPUT`. Planning refuses an existing output, binds metadata-bearing predictions to the training split/view/checkpoint and requires finite positive-area boxes within image bounds; a bare COCO prediction list can also be supplied. It does not clip malformed inputs. The controlled preparation script separately recorded clipping/discard rules, dropped 63 zero-area predictions and froze both plans before revelation. The CLI wrappers were added after the cloud source snapshot; they do not change the deployed training semantics.

A post-selection visual audit inspected every acquired no-helmet entry: 51 guided boxes in 21 images and five random boxes in three images, with all random boxes also present in guided. Nine guided boxes came from one recognizable rooftop camera sequence. No obvious hardhat/whole-body class mistake was observed in that bounded subset, but box extent varies and this is not expert adjudication. The audit changed no selections or labels and did not read model outcomes. Acquisition provenance records exposure counts without claiming scene independence.
...
The current full suite passed 154 tests. The completed acquisition panel passed real desktop/mobile browser checks, and independent rescoring reproduced all four primary metric dictionaries exactly. All seven original/continuation tiled and class-routing comparisons, all three acquisition primary runs and all three acquisition inference interactions are collected and audited. Acquisition also has real two-update CPU smoke evidence. The provider check reports zero containers across all project apps. All newly authorized experiments are complete; the nine older interrupted original-cohort runs remain explicitly incomplete.
