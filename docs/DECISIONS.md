...
Updated September 18, 2026. This is a continuing engineering record: decisions, observed failures, resulting changes and unresolved questions. A saved checkpoint means durable work and a status report; it does not end autonomous execution. [RESULTS.json](RESULTS.json) captures the current measurements. Full receipts under ignored `artifacts/` remain local.

## Persistent optimization and the next controlled experiment

The user explicitly reopened optimization after the ontology/localization follow-up and requested continuous progress. The persistent goal and [research queue](RESEARCH_LOOP.json) now carry work from one batch into the next. The runner stores reservations before submission and call IDs immediately afterward, reconnects paid work after restart, collects weights before scoring, and leaves the goal active when a batch finishes. An ambiguous submission fails closed to prevent duplicate charges. Cloud-only compute remains mandatory because local training previously froze the user's computer.

The next hypothesis targets construction sampling and scale separately. Its 995 training images are already 640×640; using native640 preserves detail lost at512, while upsampling far beyond native resolution would not recover new information. Guided supervision includes84 no-helmet boxes in40images. The previous targeted-crop experiment mixed balancing with changed scene context and regressed, so it did not isolate the effect of rare-class exposure.

Six fixed2000-update runs test an aware2×2 design (512/640 and uniform/repeat exposure), plus complete-label ordinary and combined controls. Sampling uses the LVIS class/image repeat-factor formula described in [Detectron2](https://github.com/facebookresearch/detectron2/blob/main/detectron2/data/samplers/distributed_sampler.py), with a declared threshold0.20. A fixed weighted-replacement sequence is an original implementation, not an exact reproduction of Detectron2's stochastic rounding sampler. Both uniform and weighted controls use replacement. The complete model replays the partial-derived sequence byte-for-byte; its extra labels cannot alter exposure. Stock augmentation, including its ordinary random crops, remains unchanged; no targeted object crops are added. All runs export final EMA and do not validate during training.

The actual cloud smoke completed two640px updates for both label arms with unchanged positional parameterization and identical exposure digest. Seven lightweight metadata/recovery tests passed locally without model compute. Independent source review found no critical path bug. Frozen source, parent/reference hashes, dependency lock, image ID and call IDs are saved. The training code never opens evaluation labels, but the shared cloud image physically includes the reference archive; this is logical separation, not inaccessible storage. Parent histories differ (aware ontology/Alpha1 versus full Alpha3), so only within-parent recipe effects are isolated by this new study. All outcomes will be retained; no gain is asserted while jobs are running.

Completed-call budgeting now distinguishes a conservative reservation from actual provider usage. Original ledgers remain intact. Twelve completed prior training runs took5743.05seconds; charging their maximum requested resources for that time plus300seconds overhead per call is6.0513credits, rounded up to7. Other prior categories retain their full5.80ceilings. The new matrix reserves6.92, leaving0.59against the user's confirmed20.31starting balance. The provider's observed4.1876cost for the previous fourapps corroborates available room but is not treated as a settled balance. No extra account, persistent volume or automatic retry is introduced.
...
## 2026-09-18 — Ontology, precision and foundation-model refinement

The [completed follow-up](RESEARCH_V2.md) contains twelve fixed-budget GPU training runs, eighteen same-device evaluations and six unchanged-policy tiled comparisons. Explicit object-exclusive negatives improved all-pieces AP to 74.1023 and the construction tiled result to 52.0943. The old pawn maximum79.1987 remains better than every new pawn run. Matched gains over continuing the existing coverage loss were modest: +0.3441 on all pieces and +0.1511 on tiled construction. Strong fully labeled references still lead on those tasks. More training alone explains much of the construction gain; Alpha-GIoU was not a general improvement.

Two different localization approaches were rejected before validation: a real-proposal visual cascade and a pinned pretrained SAM2.1 Large boundary refiner. Both lost to unchanged boxes on an image-grouped training holdout. The segmentation pilot also tried fixed, training-only box-padding calibration; visually tighter masks did not match the dataset annotations better. No failed method was promoted and no reference labels, confidence scores or classes were removed to inflate AP. Every outcome is preserved in [the portable evidence](RESEARCH_V2_RESULTS.json).

Local training froze the user’s computer, so it was terminated at their request. All subsequent training and model scoring ran on cloud GPUs. The unfinished local alpha1/full controls are explicitly marked interrupted/not-started, not claimed as completed comparisons. A cloud evaluator import-path deployment error was corrected by deploying its qualified package module; the original queued calls completed without resubmission. Checkpoints are persisted before independent scoring, and the published galleries now bind the newly selected weights and inference policy exactly.
