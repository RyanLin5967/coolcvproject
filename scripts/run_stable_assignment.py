...
from coveragecv.training.cloud_budget import reserve_stable_assignment
...
def invoke(folder, stage, request, arguments):
    ...
    if (folder / "call.json").exists():
        ...
        reservation = reserve_stable_assignment(stage)
        ...
        call = modal.Function.from_name("coveragecv-stable-assignment", function).spawn(**arguments)
...
def training(case, spec, protocol, sha):
    ...
    with (folder / ".lock").open("a") as lock:
            ...
            with zipfile.ZipFile(io.BytesIO(returned)) as archive:
                ...
                for name in archive.namelist():
                    if name.endswith(".json"):
                        write_json(safe_child(folder, name), json.loads(archive.read(name)))
                    else:
                        temporary = folder / "detector.pt.partial"
        ...
        if (run["status"] != "completed" or run["detector_sha256"] != checkpoint_sha
                ...
                or any(run[k] != spec[k] for k in ("steps", "arm", "alpha", "exclusive_groups", "resolution", "seed", "stable_assignment"))):
