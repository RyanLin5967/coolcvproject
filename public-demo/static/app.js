...
import {benchmarkTasks, modelRoles, matchedBenchmark} from './benchmark-view.js';
...
async function renderResearch(force=false){
 ...
 if(!benchmark){$('content').innerHTML=html+'<div class="card card-body">A complete matched comparison is not available yet.</div>';return}
 const {rows,seeds,gain,gap}=benchmark;
 html+=`<div id="benchmark-comparison" data-task="${benchmark.key}"><div class="benchmark-context"><b>${benchmark.name}</b><span>${benchmark.recipe}</span><span>${seeds.length===1?'1 run · exploratory':`${seeds.length} repeated runs · average`}</span><span>${benchmark.images} validation images</span></div>${scoreCards(rows)}<div class="benchmark-takeaway"><strong>${gain>=0?'+':''}${gain.toFixed(2)}<span> AP points</span></strong><div><b>Our method vs. ordinary training</b><p>Same incomplete labels, images and training budget.${gap>=0?` Within ${gap.toFixed(2)} points of the fully labeled reference.`:''}</p></div></div>${methodExplainer()}</div>`;
 if(benchmark.key==='construction')html+=`<p class="benchmark-note">This isolates the training method. Additional label-review experiments are separate. Construction annotations have known inconsistencies; this is a one-run validation result.</p>`;
 else html+=`<p class="benchmark-note">These chess results share a small board/camera domain. Scores from different datasets are not directly comparable.${seeds.length===1?' This larger-model result is a one-run pilot.':''}</p>`;
 html+=`<details class="disclosure"><summary>What does this score measure?</summary><div><p><b>AP50:95</b> rewards finding the right objects and drawing accurate boxes around them. It averages performance across several box-overlap requirements. A score of 78 is not “78% of images correct.”</p><p><b>AP50</b> uses a looser overlap requirement, so its number is usually higher. All three models are scored on the same validation images and reference labels.</p><p>${benchmark.steps.toLocaleString()} training updates per model · matched seeds: ${seeds.join(', ')}. The reference has more training annotations; it is a measured comparison, not a guaranteed performance ceiling.</p></div></details>`;
 const acquisition=data.acquisition;
 if(benchmark.key==='construction'&&acquisition?.status==='completed'){
  const guided=acquisition.cases.guided,random=acquisition.cases.random;
  html+=`<details class="disclosure" id="acquisition-evidence"><summary>Separate experiment: choosing what to label · +${acquisition.primary_AP_delta_points.toFixed(2)} AP</summary><div><p>We also tested which missing labels to review. Both strategies get 150 image/class reviews and the same extra training.</p><div class="table-wrap"><table><thead><tr><th>Review strategy</th><th>New boxes</th><th>AP50:95</th></tr></thead><tbody><tr><td>Random selection · control</td><td>${random.acquisition.added_boxes}</td><td>${(random.metrics.AP*100).toFixed(2)}</td></tr><tr class="aware"><td>CoverageCV guided selection · ours</td><td>${guided.acquisition.added_boxes}</td><td>${(guided.metrics.AP*100).toFixed(2)}</td></tr></tbody></table></div><p class="benchmark-note">One-run simulation using withheld published training labels. These are additional annotations, not new human work. Equal review counts do not imply equal annotation effort. This is separate from the training-only comparison above.</p></div></details>`;
 }
 html+=`<div class="benchmark-next"><p>Inspect the model outputs behind these results.</p><button class="button primary" data-page="experiments">Compare predictions →</button></div>`;
 html+=`<details class="disclosure archive"><summary>Experiment history & limitations</summary><div><p>The demo highlights one complete comparison per dataset. Other recipes—including regressions—remain in the experiment record.</p><div class="table-wrap"><table><thead><tr><th>Recorded recipe</th><th>Runs</th><th>AP50:95</th></tr></thead><tbody>${Object.values(benchmark.task.methods).map(method=>`<tr><td>${esc(method.label)}</td><td>${method.n}</td><td>${(100*method.metrics.AP.mean).toFixed(2)}</td></tr>`).join('')}</tbody></table></div><p>Some recipes used different budgets or incomplete seed cohorts; rows here are not automatically matched comparisons. Validation guided experiment design. These results do not establish state-of-the-art performance.</p><a href="https://github.com/RyanLin5967/coolcvproject/blob/main/docs/DECISIONS.md" target="_blank" rel="noopener">Read the full experiment record ↗</a></div></details>`;
